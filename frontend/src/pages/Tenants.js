...
const Tenants = () => {
  ...
  const { data } = useRTKQuery(useGetAllTenantsQuery, {
    name: searchedTerm,
    page: page,
    tenantIds: auth.tenantIds ?? "",
  });
  const { trigger: createTenant } = useRTKMutation(useCreateTenantMutation);
  const { trigger: updateTenant } = useRTKMutation(useUpdateTenantMutation);
